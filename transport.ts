...
при загрузке данных я выполняю следующие действия 
 const contractor = data.FilterContractor
        ? data.FilterContractor
        : 'Прочее';
      if (this.model.filterContractors[contractor]) {
...
а за тем использую это в компоненте функцией filterRequestsByContractor


Но есть проблема 
при смене города эти данные не обновляются 
вот функция смены города
я хочу чтобы был реализован пересчет при смене города
